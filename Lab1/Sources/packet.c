...
#include "UART.h"
 ...
 uint8_t PacketChecksum;    /*!< The Packet's Checksum */
 static  uint8_t state = 0;  /*!< The current state of the state machine */
...
{
  PacketChecksum = 0; // Initialize any values present already
...
BOOL Packet_Put(const uint8_t command, const uint8_t parameter1, const uint8_t parameter2, const uint8_t parameter3)
{
  if (UART_OutChar(command)    &&
      ...
      UART_OutChar(command^parameter1^parameter2^parameter3)) // Put the packet, byte by byte into TxFIFO
